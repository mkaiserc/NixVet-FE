...
export default function ExamsSettingsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [resList, resAreas] = await Promise.all([
        api.get<Exam[]>('/catalog/exams'),
        api.get<Area[]>('/catalog/exam-areas'),
      ]);
  ...
  const openEdit = (row: Exam) => {
    ...
    setEditingId(row.id);
    form.setFieldsValue({ name: row.name, description: row.description ?? '', area_id: row.area_id });
    setModalOpen(true);
  };
  ...
  const handleDelete = async (row: Exam) => {
    ...
    try {
      await api.delete(`/catalog/exams/${row.id}`);
  ...
  const columns = [
    ...
    { title: 'Categoria', dataIndex: ['area', 'name'], key: 'area' },
    {
      title: 'Tipo',
      key: 'type',
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-blue-600">Exames</h2>
      ...
      <Modal title={editingId != null ? 'Editar exame' : 'Novo exame'} open={modalOpen} onCancel={() => setModalOpen(false)} footer={null}>
        <Form form={form} layout="vertical" onFinish={onFinish}>
          <Form.Item name="area_id" label="Área" rules={[{ required: true }]}>
            <Select placeholder="Área" options={areas.map((a) => ({ value: a.id, label: a.name }))} />
          </Form.Item>
